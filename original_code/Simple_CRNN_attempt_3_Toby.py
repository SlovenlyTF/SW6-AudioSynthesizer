...
def build_crnn(data, labels, epoch, batchsize):
    ...
    model.add(layers.Reshape((-1, 128 * 64)))  # Flatten spatial dimensions
    ...
    model.add(layers.Reshape((32, 64, 1)))
    ...
    model.add(layers.UpSampling2D((2, 1)))
...
model = build_crnn(Input, Result, 10, 1)
