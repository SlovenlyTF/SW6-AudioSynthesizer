...
class Converter:
    ...
    def _determine_ranges(self):
        ...
        self.frequencies = np.fft.fftfreq(self.bins, 1 / self.info.samplerate)[
            : self.bins // 2
        ]
    ...
    def _reduce_freqs(self, freqs):
        ...
        reduced_freqs = [(self._freq_to_pitch(freq[0]), freq[1]) for freq in freqs]

        return reduced_freqs

    def _samples_to_freqs(self, samples):
        amplitudes = np.fft.fft(samples)
        freqs = []
        ...
        for index in range(self.min_bin, self.max_bin):
            ...
            freqs.append(
                [
                    ...
                    np.sqrt(
                        np.float_power(amplitudes[index].real, 2)
                        + np.float_power(amplitudes[index].imag, 2)
                    ),
                ]
            )
        ...
        with midi_writer.MidiWriter(
            ...
            for block in sf.blocks(
                ...
                if len(block) != self.block_size:
                    filler = np.array(
                        [
                            np.array([0.0 for _ in range(self.info.channels)])
                            for _ in range(self.block_size - len(block))
                        ]
                    )
                    block = np.append(block, filler, axis=0)
